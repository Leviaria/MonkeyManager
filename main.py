...
def getResolutionDependentData(resolution=pyautogui.size(), gamemode=""):
    ...
    requiredComparisonImages = [
        ...
        {"category": "screens", "name": "difficulty_selection"},
        {"category": "screens", "name": "gamemode_selection"},
        {"category": "screens", "name": "hero_selection"},
        {"category": "screens", "name": "ingame"},
        {"category": "screens", "name": "ingame_paused"},
        {"category": "screens", "name": "victory_summary"},
        {"category": "screens", "name": "victory"},
        {"category": "screens", "name": "defeat"},
        {"category": "screens", "name": "overwrite_save"},
        {"category": "screens", "name": "levelup"},
        {"category": "screens", "name": "apopalypse_hint"},
        {"category": "screens", "name": "round_100_insta"},
        {"category": "game_state", "name": "game_paused"},
...
def main():
    ...
    allAvailablePlaythroughs = getAllAvailablePlaythroughs(
        ["own_playthroughs"], considerUserConfig=True
    )
    ...
    if len(argv) <= iArg:
        customPrint(
            "arguments missing! Usage: py replay.py <mode> <mode arguments...> <flags>"
        )
        return
    elif argv[iArg] == "file":
        ...
        if len(argv) <= iArg + 1:
            customPrint(
                "requested running a playthrough but no playthrough provided! exiting!"
            )
            return
        ...
        if len(argv) > iAdditional + 1 and argv[iAdditional] == "continue":
            ...
            if str(argv[iAdditional + 1]) == "-":
                ...
                customPrint(
                    "continue of playthrough requested but no instruction offset provided!"
                )
                return
            customPrint("stats logging disabled!")
            logStats = False
            ...
            if len(argv) >= iAdditional + 1 and argv[iAdditional] == "until":
                if str(argv[iAdditional + 1]).isdigit():
                    ...
                    customPrint(
                        "cutting of instructions for playthrough requested but no index provided!"
                    )
                    return
                parsedArguments.append(argv[iAdditional])
                parsedArguments.append(argv[iAdditional + 1])
        ...
        if not parseBTD6InstructionFileName(argv[iArg + 1]):
            customPrint(
                '"'
                + str(argv[iArg + 1])
                + "\" can't be recognized as a playthrough filename! exiting!"
            )
            return
        elif (
            str(argv[iArg + 1]).count("/")
            or str(argv[iArg + 1]).count("\\")
            and exists(argv[iArg + 1])
        ):
            filename = argv[iArg + 1]
        elif exists("monkeymanager/own_maps/" + argv[iArg + 1]):
            filename = "monkeymanager/own_maps/" + argv[iArg + 1]
        elif exists("monkeymanager/maps/" + argv[iArg + 1]):
            filename = "monkeymanager/maps/" + argv[iArg + 1]
        elif exists("monkeymanager/unvalidated_maps/" + argv[iArg + 1]):
            filename = "monkeymanager/unvalidated_maps/" + argv[iArg + 1]
        else:
            customPrint(
                "requested playthrough " + str(argv[iArg + 1]) + " not found! exiting!"
            )
            return
        mapConfig = parseBTD6InstructionsFile(filename, gamemode=gamemode)

        segmentCoordinates = getResolutionDependentData(
            resolution, mapConfig["gamemode"]
        )["segmentCoordinates"]
        ...
        if instructionOffset == -1:
            ...
            if "hero" in mapConfig:
                originalObjectives.append(
                    {"type": State.SELECT_HERO, "mapConfig": mapConfig}
                )
                originalObjectives.append({"type": State.GOTO_HOME})
            originalObjectives.append(
                {"type": State.GOTO_INGAME, "mapConfig": mapConfig}
            )
        else:
            if instructionOffset >= len(mapConfig["steps"]) or (
                instructionLast != -1 and instructionOffset >= instructionLast
            ):
                customPrint(
            ...
            if instructionLast != -1:
                mapConfig["steps"] = mapConfig["steps"][
                    (
                        instructionOffset + mapConfig["extrainstructions"]
                    ) : instructionLast
                ]
            else:
                mapConfig["steps"] = mapConfig["steps"][
        ...
        if len(argv) <= iArg + 1 or not argv[iArg + 1] in locateImages["collection"]:
            customPrint(
                "requested chasing event rewards but no event specified or unknown event! exiting!"
            )
            return
        ...
        if collectionEvent == "golden_bloon":
            ...
            customPrint(
                f"Mode: playing games with increased {collectionEvent} collection event rewards"
                + (f" on {gamemodeRestriction}" if gamemodeRestriction else "")
                + (f" in {categoryRestriction} category" if categoryRestriction else "")
                + "!"
            )
        ...
        allAvailablePlaythroughsList = sortPlaythroughsByXPGain(
            allAvailablePlaythroughsList
        )

        if len(argv) > iArg + 1 and argv[iArg + 1].isdigit():
            allAvailablePlaythroughsList = allAvailablePlaythroughsList[
                : int(argv[iArg + 1])
            ]
            parsedArguments.append(argv[iArg + 1])
        else:
        ...
        allAvailablePlaythroughsList = sortPlaythroughsByMonkeyMoneyGain(
            allAvailablePlaythroughsList
        )
        ...
        if argv[iArg + 1] == "file":
            ...
            if not parseBTD6InstructionFileName(argv[iArg + 2]):
                ...
                str(argv[iArg + 1]).count("/")
                or str(argv[iArg + 2]).count("\\")
                and exists(argv[iArg + 2])
            ):
                filename = argv[iArg + 1]
            elif exists("monkeymanager/own_maps/" + argv[iArg + 2]):
                filename = "monkeymanager/own_maps/" + argv[iArg + 2]
            elif exists("monkeymanager/maps/" + argv[iArg + 2]):
                filename = "monkeymanager/maps/" + argv[iArg + 2]
            elif exists("monkeymanager/unvalidated_maps/" + argv[iArg + 2]):
                filename = "monkeymanager/unvalidated_maps/" + argv[iArg + 2]
            else:
                customPrint(
                    "requested playthrough "
                    + str(argv[iArg + 2])
                    + " not found! exiting!"
                )
                return
            ...
            allAvailablePlaythroughsList = [
                {
                    ...
                    "gamemode": fileConfig["gamemode"],
                    "isOriginalGamemode": True,
                }
            ]
            ...
            customPrint(
                "Mode: validating all playthroughs"
                + (
                    " in " + categoryRestriction + " category"
            ...
            allAvailablePlaythroughs = filterAllAvailablePlaythroughs(
                ...
                (
                    ...
                    if handlePlaythroughValidation == ValidatedPlaythroughs.INCLUDE_ALL
                    else ValidatedPlaythroughs.INCLUDE_ALL
                ),
                categoryRestriction,
                gamemodeRestriction,
                onlyOriginalGamemodes=True,
            )
            allAvailablePlaythroughsList = allPlaythroughsToList(
                allAvailablePlaythroughs
            )
        ...
        if getMonkeyKnowledgeStatus():
            customPrint(
                "Mode validate costs only works with monkey knowledge disabled!"
            )
            return
        ...
        customPrint(
            "Mode: validating monkey costs"
            + (" including heros" if includeHeros else "")
            + "!"
        )
        ...
        if getResolutionString() in allTestPositions:
            ...
            testPositions = json.loads(
                convertPositionsInString(
                    json.dumps(allTestPositions["2560x1440"]),
                    (2560, 1440),
                    pyautogui.size(),
                )
            )
        ...
        if selectedMap is None:
            customPrint(
                'This mode requires access to medium sandbox for one of the maps in "test_positions.json"!'
            )
            return
                ...
                monkeySteps.append(
                    {
                        ...
                        "key": keybinds["monkeys"][monkeyType],
                        "pos": pos[towers["monkeys"][monkeyType]["class"]],
                        "cost": 1,
                        "extra": {"group": "monkeys", "type": monkeyType},
                    }
                )
                        ...
                        monkeySteps.append(
                            {
                                "action": "click",
                                "name": f"{monkeyType}{iPath}",
                                "pos": imageAreas["click"][
                                    "paragon_message_confirmation"
                                ],
                                "cost": 0,
                            }
                        )
        ...
        originalObjectives.append({"type": State.GOTO_HOME})
        originalObjectives.append(
            {"type": State.GOTO_INGAME, "mapConfig": monkeyMapConfig}
        )
                ...
                heroMapConfig["steps"] = [
                    {
                        "action": "click",
                        "pos": imageAreas["click"][
                            "gamemode_deflation_message_confirmation"
                        ],
                        "cost": 0,
                    },
                    {
                        ...
                        "pos": pos[towers["heros"][hero]["class"]],
                        "cost": 1,
                        "extra": {"group": "heros", "type": hero},
                    },
                ]
                originalObjectives.append({"type": State.GOTO_HOME})
                originalObjectives.append(
                    {"type": State.SELECT_HERO, "mapConfig": heroMapConfig}
                )
                originalObjectives.append({"type": State.GOTO_HOME})
                originalObjectives.append(
                    {"type": State.GOTO_INGAME, "mapConfig": heroMapConfig}
                )
                originalObjectives.append(
                    {"type": State.INGAME, "mapConfig": heroMapConfig}
                )
    ...
    for arg in sys.argv:
        if len(np.where(parsedArgumentsTmp == arg)[0]):
            parsedArgumentsTmp = np.delete(
                parsedArgumentsTmp, np.where(parsedArgumentsTmp == arg)[0]
            )
        else:
        ...
        if usesAllAvailablePlaythroughsList:
            ...
            for playthrough in allAvailablePlaythroughsList:
                customPrint(
                    ...
                    + (
                        " with "
                        + str(playthrough["value"])
                        + (" " + valueUnit if len(valueUnit) else "")
                        if "value" in playthrough
                        else ""
                    )
                )
        else:
    ...
    while True:
        ...
        if not activeWindow or not isBTD6Window(activeWindow.title):
            ...
            for screenCfg in [
                ...
                (
                    Screen.ROUND_100_INSTA,
                    comparisonImages["screens"]["round_100_insta"],
                    imageAreas["compare"]["screens"]["round_100_insta"],
                ),
                (
                ...
                diff = cv2.matchTemplate(
                    cutImage(screenshot, screenCfg[2]),
        ...
        if screen == Screen.BTD6_UNFOCUSED:
            ...
            if mode == Mode.VALIDATE_PLAYTHROUGHS:
                if validationResult != None:
                    customPrint(
                        ...
                        + " is "
                        + ("valid" if validationResult else "invalid")
                        + "!"
                    )
                    updatePlaythroughValidationStatus(
                        lastPlaythrough["filename"], validationResult
                    )
                if len(allAvailablePlaythroughsList):
                    ...
                    if gamemode:
                        mapConfig = parseBTD6InstructionsFile(
                            playthrough["filename"], gamemode=gamemode
                        )
                        objectives = []
                        objectives.append({"type": State.GOTO_HOME})
                        if (
                            "hero" in mapConfig
                            and lastHeroSelected != mapConfig["hero"]
                        ):
                            objectives.append(
                                {"type": State.SELECT_HERO, "mapConfig": mapConfig}
                            )
                        ...
                        objectives.append(
                            {"type": State.INGAME, "mapConfig": mapConfig}
                        )
                ...
                for monkeyType in costs["monkeys"]:
                    if (
                        costs["monkeys"][monkeyType]["base"]
                        and costs["monkeys"][monkeyType]["base"]
                        != oldTowers["monkeys"][monkeyType]["base"]
                    ):
                        print(
                            f"{monkeyType} base cost: {oldTowers['monkeys'][monkeyType]['base']} -> {int(costs['monkeys'][monkeyType]['base'])}"
                        )
                        towers["monkeys"][monkeyType]["base"] = int(
                            costs["monkeys"][monkeyType]["base"]
                        )
                        changes += 1
                    for iPath in range(0, 3):
                        for iUpgrade in range(0, 5):
                            if (
                                costs["monkeys"][monkeyType]["upgrades"][iPath][
                                    iUpgrade
                                ]
                                and costs["monkeys"][monkeyType]["upgrades"][iPath][
                                    iUpgrade
                                ]
                                != oldTowers["monkeys"][monkeyType]["upgrades"][iPath][
                                    iUpgrade
                                ]
                            ):
                                print(
                                    f"{monkeyType} path {iPath + 1} upgrade {iUpgrade + 1} cost: {oldTowers['monkeys'][monkeyType]['upgrades'][iPath][iUpgrade]} -> {int(costs['monkeys'][monkeyType]['upgrades'][iPath][iUpgrade])}"
                                )
                                towers["monkeys"][monkeyType]["upgrades"][iPath][
                                    iUpgrade
                                ] = int(
                                    costs["monkeys"][monkeyType]["upgrades"][iPath][
                                        iUpgrade
                ...
                if "heros" in costs:
                    for hero in costs["heros"]:
                        if (
                            costs["heros"][hero]["base"]
                            and costs["heros"][hero]["base"]
                            != oldTowers["heros"][hero]["base"]
                        ):
                            print(
                                f"hero {hero} base cost: {oldTowers['heros'][hero]['base']} -> {int(costs['heros'][hero]['base'])}"
                            )
                            towers["heros"][hero]["base"] = int(
                                costs["heros"][hero]["base"]
                            )
                            changes += 1
                ...
                if mode == Mode.SINGLE_MAP:
                    ...
                    mode == Mode.RANDOM_MAP
                    or mode == Mode.XP_FARMING
                    or mode == Mode.MM_FARMING
                ):
                    objectives = []
                    ...
                    mapConfig = parseBTD6InstructionsFile(
                        playthrough["filename"], gamemode=playthrough["gamemode"]
                    )
                    ...
                    if increasedRewardsPlaythrough:
                        ...
                        objectives.append({"type": State.GOTO_HOME})
                        objectives.append(
                            {"type": State.FIND_HARDEST_INCREASED_REWARDS_MAP}
                        )
                        objectives.append({"type": State.MANAGE_OBJECTIVES})
                else:
                    objectives = copy.deepcopy(originalObjectives)
            ...
            if screen == Screen.STARTMENU:
                ...
                result = cv2.matchTemplate(
                    screenshot, locateImages["button_home"], cv2.TM_SQDIFF_NORMED
                )
                pyautogui.click(cv2.minMaxLoc(result)[2])
            elif screen == Screen.VICTORY_SUMMARY:
                pyautogui.click(
                    imageAreas["click"]["screen_victory_summary_button_next"]
                )
                ...
                while True:
                    ...
                    result = [
                        cv2.minMaxLoc(
                            cv2.matchTemplate(
                                newScreenshot,
                                locateImages["unknown_insta"],
                                cv2.TM_SQDIFF_NORMED,
                                mask=locateImages["unknown_insta_mask"],
                            )
                        )[i]
                        for i in [0, 2]
                    ]
            ...
            if screen == Screen.STARTMENU:
                ...
                if mapConfig["category"] == "beginner":
                    ...
                    time.sleep(menuChangeDelay)
                    pyautogui.click(
                        imageAreas["click"]["map_categories"][mapConfig["category"]]
                    )
                    time.sleep(menuChangeDelay)
                else:
                ...
                time.sleep(menuChangeDelay)
                pyautogui.click(
                    imageAreas["click"]["gamedifficulty_positions"][
                ...
                if logStats:
                    lastPlaythroughStats = {
                        ...
                        "time": [],
                        "result": PlaythroughResult.UNDEFINED,
                    }
            ...
            if screen == Screen.STARTMENU:
                pyautogui.click(
                    imageAreas["click"]["screen_startmenu_button_hero_selection"]
                )
                time.sleep(menuChangeDelay)
                pyautogui.click(
                    imageAreas["click"]["hero_positions"][mapConfig["hero"]]
                )
                time.sleep(menuChangeDelay)
                pyautogui.click(
            ...
            if screen == Screen.STARTMENU:
                ...
                if categoryRestriction:
                    pyautogui.click(
                        imageAreas["click"]["map_categories"][
                            (
                                "advanced"
                                if categoryRestriction == "beginner"
                                else "beginner"
                            )
                        ]
                    )
                    ...
                    for page in range(0, categoryPages[categoryRestriction]):
                        pyautogui.click(
                            imageAreas["click"]["map_categories"][categoryRestriction]
                        )
                        if collectionEvent == "golden_bloon":
                            time.sleep(4)
                        else:
                            time.sleep(menuChangeDelay)
                        newScreenshot = np.array(pyautogui.screenshot())[
                            :, :, ::-1
                        ].copy()
                        result = findImageInImage(
                            newScreenshot, locateImages["collection"][collectionEvent]
                        )
                        if result[0] < 0.05:
                            mapname = findMapForPxPos(
                                categoryRestriction, page, result[1]
                            )
                            break
                    if not mapname:
                    ...
                    for category in reversed(list(mapsByCategory.keys())):
                        ...
                        for page in range(0, categoryPages[category]):
                            pyautogui.click(
                                imageAreas["click"]["map_categories"][category]
                            )
                            if collectionEvent == "golden_bloon":
                                time.sleep(4)
                            else:
                            ...
                            result = findImageInImage(
                                newScreenshot,
                                locateImages["collection"][collectionEvent],
                            )
                            if result[0] < 0.05:
            ...
            if screen == Screen.INGAME_PAUSED:
                ...
                if not mapConfig["gamemode"] in playthroughLog[mapConfig["filename"]]:
                    playthroughLog[mapConfig["filename"]][mapConfig["gamemode"]] = {
                        "attempts": 0,
                        "wins": 0,
                        "defeats": 0,
                    }
                    ...
                    if (
                        ...
                        if currentValues["money"] == lastIterationBalance:
                            ...
                            customPrint(
                                ...
                                + ", detected cost: "
                                + str(lastIterationBalance - currentValues["money"])
                                + ". Is monkey knowledge disabled?"
                            )
                elif mode == Mode.VALIDATE_COSTS:
                    if lastIterationBalance != -1 and lastIterationAction:
                        if lastIterationAction["action"] == "place":
                            costs[lastIterationAction["extra"]["group"]][
                                lastIterationAction["extra"]["type"]
                            ]["base"] = int(
                            ...
                            costs[lastIterationAction["extra"]["group"]][
                                lastIterationAction["extra"]["type"]
                            ]["upgrades"][lastIterationAction["extra"]["upgrade"][0]][
                                lastIterationAction["extra"]["upgrade"][1] - 1
                            ] = int(
                                lastIterationBalance - currentValues["money"]
                            )
                ...
                if (
                    ...
                    and (
                        mapConfig["steps"][0]["action"] == "await_round"
                        or mapConfig["steps"][0]["action"] == "speed"
                    )
                ):
                    mapConfig["steps"].pop(0)
                elif (
                    currentValues["money"] == -1
                    or currentValues["round"] == -1
                    and len(mapConfig["steps"])
                    ...
                    mode != Mode.VALIDATE_COSTS
                    and (
                        currentValues["round"] - lastIterationRound > 1
                        or lastIterationRound > currentValues["round"]
                    )
                    and len(mapConfig["steps"])
                    and mapConfig["steps"][0]["action"] == "await_round"
                ):
                    ...
                    (
                        mapConfig["steps"][0]["action"] != "sell"
                        and mapConfig["steps"][0]["action"] != "await_round"
                        and min(
                            currentValues["money"],
                            lastIterationBalance - lastIterationCost,
                        )
                        >= mapConfig["steps"][0]["cost"]
                    )
                    ...
                    if action["action"] == "place":
                        ...
                        while action:
                            ...
                            if (
                                "to" in action
                                and mapConfig["monkeys"][action["name"]]["type"]
                                == "mortar"
                            ):
                                pyautogui.click()
                            time.sleep(smallActionDelay)
                            ...
                            if (
                                len(mapConfig["steps"])
                                and "name" in mapConfig["steps"][0]
                                and mapConfig["steps"][0]["name"] == action["name"]
                                and (
                                    mapConfig["steps"][0]["action"] == "retarget"
                                    or mapConfig["steps"][0]["action"] == "special"
                                    or mapConfig["steps"][0]["action"] == "click"
                                )
                            ):
                                action = mapConfig["steps"].pop(0)
                        ...
                        time.sleep(menuChangeDelay)
                        result = cv2.matchTemplate(
                            np.array(pyautogui.screenshot())[:, :, ::-1].copy(),
                            locateImages["remove_obstacle_confirm_button"],
                            cv2.TM_SQDIFF_NORMED,
                        )
                        pyautogui.click(cv2.minMaxLoc(result)[2])
                    ...
                    mode in [Mode.VALIDATE_PLAYTHROUGHS, Mode.VALIDATE_COSTS]
                    and len(mapConfig["steps"]) == 0
                    and lastIterationCost == 0
                ):
                    state = State.UNDEFINED

                if (
                    not doAllStepsBeforeStart
                    and mapConfig["gamemode"] != "deflation"
                    and not skippingIteration
                    ...
                    for screenCfg in [
                        (
                            "game_playing_fast",
                            comparisonImages["game_state"]["game_playing_fast"],
                            imageAreas["compare"]["game_state"],
                        ),
                        (
                            "game_playing_slow",
                            comparisonImages["game_state"]["game_playing_slow"],
                            imageAreas["compare"]["game_state"],
                        ),
                        (
                            "game_paused",
                            comparisonImages["game_state"]["game_paused"],
                            imageAreas["compare"]["game_state"],
                        ),
                    ]:
                        diff = cv2.matchTemplate(
